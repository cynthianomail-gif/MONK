...
**每個路人生 2 張走路 frame（`_a`＝左腳前、`_b`＝右腳前），我 code 交替播成踏步**（避免靜圖紙片滑行感）。同一人、同衣、只換腳/手擺動。

| 檔名(`minigames/` 下) | 是誰 | 內容 | 尺寸 |
|---|---|---|---|
| `beggar_ped_office_worker_a.png` ＋ `_b.png` | 上班族(常見) | 日本上班族西裝、公事包、趕路 | ~512×900 直幅全身 |
| `beggar_ped_tourist_a.png` ＋ `_b.png` | 觀光客 | 背包+相機、悠閒 | ~512×900 |
| `beggar_ped_rich_lady_a.png` ＋ `_b.png` | 貴婦(稀有/高賞) | 華麗和服貴婦、貴氣 | ~512×900 |
| `beggar_ped_drunk_man_a.png` ＋ `_b.png` | 醉漢(低賞) | 微醺踉蹌、酒瓶 | ~512×900 |
| `beggar_wujie_begging.png`（單張靜態） | 乞討無戒 | 無戒**側面**托缽(持缽蹲/站)，同 cast 黑袈裟 | ~512×900 |

> 重點：**側視＋面朝左＋透明底**(跟 cast 正面立繪不同角度)。**4 路人各 2 frame**(_a/_b 兩個踏步姿，腳/手交替；身體可些微上下)，乞討無戒靜態單張。共 **4×2 + 1 = 9 張**。
