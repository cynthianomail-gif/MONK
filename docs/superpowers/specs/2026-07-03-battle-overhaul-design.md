...
版面（mockup 已過目）：
- **TurnOrderBar**（左上）：行動順序圓 chip 橫排，當前行動者放大＋朱紅底＋金邊；下一輪的用虛線邊。
...
1. **行動佇列**：`Combatant.speed` 新欄位；`enemies.json` 每敵補 `speed`（暫值＝10＋level×2，第四期調）；玩家 speed 基礎 15＋修行盤加成。回合開始把所有存活戰鬥者按 speed 降冪排成佇列，依序行動（取代固定我方→敵方）；One More 規則不變（玩家弱點命中→插入一次額外行動）。TurnOrderBar 讀佇列渲染。
